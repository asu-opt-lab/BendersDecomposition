...
@testset verbose = true "CFLP Callback DiscorverNorm Tests" begin
    ...
    instances = discorvernorm_benchmark_instances()

    for i in instances
        @testset "Instance: p$i" begin
            data = read_cflp_benchmark_data("p$i")
            support_info = build_discorvernorm_support_points(data; t_margin_rel = 0.05, t_margin_abs = 1.0, optimizer = optimizer)
            ...
            @testset "Classical oracle" begin
                oracle_param = DiscorverNormDCGLPParam(
                    dcglp_param,
                    support_info.support_points_x,
                    support_info.support_points_t;
                ...
                @testset "NoSeq" begin
                    @info "solving DiscorverNormDCGLP CFLP p$i - callback - classical/no seq"
                ...
                @testset "Seq" begin
                    @info "solving DiscorverNormDCGLP CFLP p$i - callback - classical/seq"
                    master = Master(data; customize = customize_master_model!, optimizer = mip_optimizer)
                    lazy_oracle = ClassicalOracle(data, master; customize = customize_sub_model!, optimizer = optimizer)
                    typical_oracles = [
                        ClassicalOracle(data, master; customize = customize_sub_model!, optimizer = optimizer),
                ...
                @testset "SeqInOut" begin
                    @info "solving DiscorverNormDCGLP CFLP p$i - callback - classical/seqinout"
                    master = Master(data; customize = customize_master_model!, optimizer = mip_optimizer)
                    lazy_oracle = ClassicalOracle(data, master; customize = customize_sub_model!, optimizer = optimizer)
                    typical_oracles = [
                        ClassicalOracle(data, master; customize = customize_sub_model!, optimizer = optimizer),
            ...
            @testset "Knapsack oracle" begin
                ...
                @testset "NoSeq" begin
                    @info "solving DiscorverNormDCGLP CFLP p$i - callback - knapsack/no seq"
                    master = Master(data; customize = customize_master_model!, optimizer = mip_optimizer)
                ...
                @testset "Seq" begin
                    @info "solving DiscorverNormDCGLP CFLP p$i - callback - knapsack/seq"
                    master = Master(data; customize = customize_master_model!, optimizer = mip_optimizer)
                    lazy_oracle = CFLKnapsackOracle(data, master; customize = customize_sub_model!, optimizer = optimizer)
                    typical_oracles = [
                        CFLKnapsackOracle(data, master; customize = customize_sub_model!, optimizer = optimizer),
                ...
                @testset "SeqInOut" begin
                    @info "solving DiscorverNormDCGLP CFLP p$i - callback - knapsack/seqinout"
                    master = Master(data; customize = customize_master_model!, optimizer = mip_optimizer)
                    lazy_oracle = CFLKnapsackOracle(data, master; customize = customize_sub_model!, optimizer = optimizer)
                    typical_oracles = [
                        CFLKnapsackOracle(data, master; customize = customize_sub_model!, optimizer = optimizer),
