...
function orginal_problem(c, d, A, B, b)
    m = Model(Gurobi.Optimizer)
    set_optimizer_attribute(m, MOI.Silent(),true)
    @variable(m, x[1:2], Bin)
    @variable(m, y[1:4]>=0)
    @constraint(m, A*x + B*y .>= b)
    @objective(m, Min, c'x + d'y)
    optimize!(m)
    @info objective_value(m), value.(x)
    return m
end
orginal_problem(c, d, A, B, b)
function master(c)
    m = Model(Gurobi.Optimizer)
    set_optimizer_attribute(m, MOI.Silent(),true)
    num = length(c)
...
function run_Benders(c, d, A, B, b)
    ...
    while true
        ...
        if status == FEASIBLE_POINT
            ...
            ex = @expression(master_problem, -master_problem[:t] + extreme_point'master_problem[:x] + const_term )
