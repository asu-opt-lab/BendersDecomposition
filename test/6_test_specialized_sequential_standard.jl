        ...
        @testset "Instance: p$i" begin
            ...
            @testset "Classical oracle" begin
                @testset "SpecialSeq" begin        
                    ...
                    # for strengthened in [false], add_benders_cuts_to_master in [false], reuse_dcglp in [true], p in [1.0] #fail
                        @info "solving p$i - begin oracle - Specialized seq - strgthnd $strengthened; benders2master $add_benders_cuts_to_master reuse $reuse_dcglp"
                        @testset "strgthnd $strengthened; benders2master $add_benders_cuts_to_master reuse $reuse_dcglp" begin
            ...
            @testset "fat knapsack oracle" begin
                @testset "SpecialSeq" begin
                    for strengthened in [false], add_benders_cuts_to_master in [false], reuse_dcglp in [false]
                    # for strengthened in [true; false], add_benders_cuts_to_master in [true; false], reuse_dcglp in [true; false], p in [1.0; Inf] 
                        @info "solving p$i - fat Knapsack oracle - Specialized seq - strgthnd $strengthened; benders2master $add_benders_cuts_to_master reuse $reuse_dcglp"
                        @testset "strgthnd $strengthened; benders2master $add_benders_cuts_to_master reuse $reuse_dcglp" begin
                            master = Master(data; solver_param = master_solver_param)
                            update_model!(master, data)
