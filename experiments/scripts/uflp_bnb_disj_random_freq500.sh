...
EXPERIMENT_VERSION="750_supplemental"
SEED="1"
HOUR="04"
TIME_LIMIT="14400"
...
FILE_NAME="uflp_bnb_disj_random_freq500.jl"
SHELL_FILE_NAME="uflp_bnb_disj_random_freq500.sh"
THREADS=7
...
cp "experiments/scripts/${SHELL_FILE_NAME}" "${OUTPUT_DIR}/${SHELL_FILE_NAME}"
...
instances=(
    ...
    "ga750a-1" "gs750a-1" "gs750a-3" "gs750a-4"
)
...
for instance in "${instances[@]}"; do
    ...
    echo "julia --project=experiments/scripts experiments/scripts/${FILE_NAME} --instance=${instance} --seed=${SEED} --time_limit=${TIME_LIMIT} --dcglp_time_limit=${DCGLP_TIME_LIMIT} --dcglp_iter_limit=${DCGLP_ITER_LIMIT} --dcglp_halt_limit=${DCGLP_HALT_LIMIT} --frequency=${FREQUENCY} --threads=${THREADS} --reuse_dcglp=${REUSE_DCGLP} --strengthened=${STRENGTHENED} --lift=${LIFT}" >> "${JOBSCRIPT_FILE}"

    sbatch --partition=general "${JOBSCRIPT_FILE}"
    rm "${JOBSCRIPT_FILE}"
done
