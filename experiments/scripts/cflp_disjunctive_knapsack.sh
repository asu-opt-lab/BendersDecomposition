...
ROUND_DESCRIPTION="CFLP Benders BnB, disjunctive knapsack oracle, 7 private cores"
EXPERIMENT_VERSION="1"
SEED="1"
HOUR="04"
TIME_LIMIT=$((10#${HOUR} * 3600))
FREQUENCY=250
EXPERIMENT_DESCRIPTION="${HOUR} hr, seed = ${SEED}, frequency = ${FREQUENCY}"

FILE_NAME="cflp_disjunctive_knapsack.jl"
THREADS=7
...
for instance in "${instances[@]}"; do
    ...
    echo "julia --project=experiments/scripts experiments/scripts/${FILE_NAME} --instance ${instance} --output_dir ${OUTPUT_DIR} --seed ${SEED} --time_limit ${TIME_LIMIT} --frequency ${FREQUENCY}" >> "${JOBSCRIPT_FILE}"
