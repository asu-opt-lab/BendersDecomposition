...
ROUND_VERSION="cflp_classical"
ROUND_DESCRIPTION="CFLP classical BendersBnB + LazyCallback + BendersSeq root with CPLEX"
EXPERIMENT_VERSION="1"
SEED="1"
HOUR="01"
TIME_LIMIT="3600"
ROOT_TIME_LIMIT="3600"
EXPERIMENT_DESCRIPTION="${HOUR} hr, seed = ${SEED}, root classical seq ${ROOT_TIME_LIMIT}s"
...
cp -r experiments/scripts/${SHELL_FILE_NAME} "${OUTPUT_DIR}/${SHELL_FILE_NAME}"
...
for instance in "${instances[@]}"; do
    ...
    echo "julia --project=experiments/scripts experiments/scripts/${FILE_NAME} --instance ${instance} --time_limit ${TIME_LIMIT} --root_time_limit ${ROOT_TIME_LIMIT} --output_dir ${OUTPUT_DIR}" >> "${JOBSCRIPT_FILE}"
