mutable struct SimplexNormOldDCGLPParam <: BendersX.AbstractOracleParam
    ...
    function SimplexNormOldDCGLPParam(
        dcglp_param::BendersX.DcglpParam;
...
mutable struct SimplexNormOldDCGLPOracle <: BendersX.AbstractDisjunctiveOracle
    param::SimplexNormOldDCGLPParam
    dcglp::Model
    ...
    function SimplexNormOldDCGLPOracle(
        master::BendersX.AbstractMaster,
        typical_oracles::Vector{T},
        param::SimplexNormOldDCGLPParam,
    ) where {T <: BendersX.AbstractTypicalOracle}
        length(typical_oracles) == 2 ||
            throw(ArgumentError("SimplexNormOldDCGLPOracle requires exactly two typical oracles."))

        for xi in master.x
            is_binary(xi) || throw(ArgumentError("SimplexNormOldDCGLPOracle requires all master variables to be binary."))
        end

        dcglp = build_simplex_norm_old_dcglp(master, param)
...
function build_simplex_norm_old_dcglp(master::BendersX.AbstractMaster, param::SimplexNormOldDCGLPParam)
    ...
    @constraint(dcglp, [i in 1:2], omega_t[i, :] .>= SIMPLEXNORMOLD_T_LOWER_BOUND .* omega_0[i])
...
function get_split_index(oracle::SimplexNormOldDCGLPOracle)
...
function replace_disjunctive_inequality!(oracle::SimplexNormOldDCGLPOracle)
    dcglp = oracle.dcglp
