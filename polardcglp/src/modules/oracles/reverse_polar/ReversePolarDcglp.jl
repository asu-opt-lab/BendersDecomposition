function BendersX.solve_dcglp!(
    ...
    while true
            ...
            state.master_time = @elapsed begin
                ...
                try
                    ...
                    return fallback_typical_or_throw(
                        ...
                        start_time,
                        time_limit,
                        "ReversePolarDCGLP master: unexpected error encountered when optimizing dcglp master: $(err)";
                ...
                elseif termination_status(dcglp) == ALMOST_INFEASIBLE
                    return fallback_typical_or_throw(
                        ...
                        start_time,
                        time_limit,
                        "ReversePolarDCGLP master: unexpected dcglp master termination status: $(termination_status(dcglp)); the problem is infeasible or dcglp encountered numerical issue";
                        throw_typical_cuts_for_errors = throw_typical_cuts_for_errors,
                    )
                elseif termination_status(dcglp) == TIME_LIMIT
                    throw(BendersX.TimeLimitException("Time limit reached during ReversePolarDCGLP solving"))
                else
                    throw(BendersX.UnexpectedModelStatusException("ReversePolarDCGLP master: $(termination_status(dcglp))"))
                end
            end
        ...
        oracle.param.dcglp_param.verbose && print_reverse_polar_iteration_info(state, log)
    ...
    if current_lb >= oracle.param.zero_tol
        cut = generate_reverse_polar_disjunctive_cut(
            dcglp,
            current_lb,
            x_value,
            t_value,
...
function generate_reverse_polar_disjunctive_cut(
    dcglp::Model,
    current_lb::Float64,
    x_value::Vector{Float64},
    t_value::Vector{Float64},
...
function print_reverse_polar_iteration_info(state::BendersX.DcglpState, log::BendersX.DcglpLog)
    @printf(
