...
struct SimplexNormTestData <: AbstractData
    open_cost::Vector{Float64}
    ship_cost::Matrix{Float64}
end

simplex_norm_test_data() = SimplexNormTestData(
    [0.5, 0.6, 0.8],
    [1.0 3.0 2.0 4.0;
     2.0 1.0 2.5 2.0;
     3.0 2.0 1.0 1.5],
...
simplex_norm_test_highs_optimizer() = optimizer_with_attributes(HiGHS.Optimizer, RMOI.Silent() => true)

function customize_master_simplex_norm_test!(model::Model, data::SimplexNormTestData)
    set_optimizer(model, simplex_norm_test_highs_optimizer())
...
function customize_sub_simplex_norm_test!(model::Model, data::SimplexNormTestData, scen_idx::Int; x)
    set_optimizer(model, simplex_norm_test_highs_optimizer())
...
function solve_simplex_norm_test_reference(data::SimplexNormTestData)
    model = Model()
    set_optimizer(model, simplex_norm_test_highs_optimizer())
    n_facilities, n_customers = size(data.ship_cost)
    @variable(model, x[1:n_facilities], Bin)
...
function make_simplex_norm_test_typical_oracles(data::SimplexNormTestData, master::Master)
    return [
        ClassicalOracle(data, master; customize = customize_sub_simplex_norm_test!),
        ClassicalOracle(data, master; customize = customize_sub_simplex_norm_test!),
    ]
end

function make_simplex_norm_test_oracle(
    data::SimplexNormTestData,
    master::Master;
    ...
    dcglp_param = DcglpParam(
        simplex_norm_test_highs_optimizer();
        time_limit = 10.0,
        gap_tolerance = 1e-8,
        halt_limit = 3,
        iter_limit = 25,
    ...
    param = SimplexNormTestDCGLPParam(
        dcglp_param;
    ...
    return SimplexNormTestDCGLPOracle(master, make_simplex_norm_test_typical_oracles(data, master), param)
end

struct SimplexNormTestVectorTData <: AbstractData end

struct SimplexNormTestVectorTOracle <: BendersX.AbstractTypicalOracle end

function customize_master_simplex_norm_test_vector_t!(model::Model, data::SimplexNormTestVectorTData)
    set_optimizer(model, simplex_norm_test_highs_optimizer())
...
@testset "SimplexNormTestDCGLP Environment" begin
    @testset "Constructor Validation" begin
        data = simplex_norm_test_data()
        master = Master(data; customize = customize_master_simplex_norm_test!)
        oracle = make_simplex_norm_test_oracle(data, master)
        @test oracle isa SimplexNormTestDCGLPOracle
        @test length(oracle.dcglp[:tau]) == 1
    end

    @testset "Direct Vector-T Cut Generation" begin
        vt_data = SimplexNormTestVectorTData()
        vt_master = Master(vt_data; customize = customize_master_simplex_norm_test_vector_t!)
        dcglp_param = DcglpParam(
            simplex_norm_test_highs_optimizer();
            time_limit = 10.0,
            gap_tolerance = 1e-8,
        ...
        oracle = SimplexNormTestDCGLPOracle(vt_master, [SimplexNormTestVectorTOracle(), SimplexNormTestVectorTOracle()], param)
    ...
    @testset "BendersSeq Solve Path" begin
        data = simplex_norm_test_data()
        reference_obj = solve_simplex_norm_test_reference(data)

        master = Master(data; customize = customize_master_simplex_norm_test!)
        oracle = make_simplex_norm_test_oracle(data, master; reuse_dcglp = true)
        env = BendersSeq(
            master,
            oracle;
