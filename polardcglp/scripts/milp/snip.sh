...
ROUND_VERSION="milp/snip"
ROUND_DESCRIPTION="SNIP MILP, 7 private cores, CPLEX"
...
for instance_no in "${instance_nos[@]}"; do
    for snip_no in "${snip_nos[@]}"; do
        for budget in "${budgets[@]}"; do
            ...
            echo "julia --project=polardcglp polardcglp/scripts/milp/${FILE_NAME} --instance_no=${instance_no} --snip_no=${snip_no} --budget=${budget} --seed=${SEED} --time_limit=${TIME_LIMIT} --output_dir=${OUTPUT_DIR}" >> "${JOBSCRIPT_FILE}"
