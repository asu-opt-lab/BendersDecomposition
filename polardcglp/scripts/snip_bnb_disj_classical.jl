...
@info "PolarDCGLP SNIP script" instance_no = instance_no snip_no = snip_no budget = budget seed = seed time_limit = time_limit frequency = frequency threads = threads reuse_dcglp = reuse_dcglp strengthened = strengthened lift = lift build_only = build_only
...
if build_only
    @info "PolarDCGLP SNIP script build completed without solve." instance_no = instance_no snip_no = snip_no budget = budget
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "PolarDCGLP SNIP script finished" instance_no = instance_no snip_no = snip_no budget = budget termination_status = env.termination_status objective_value = obj_value
end
