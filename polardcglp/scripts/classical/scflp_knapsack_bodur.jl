...
instance = get_string_option(options, "instance", "cap101-s250")
...
@info "SCFLP knapsack script (CPLEX, Bodur formulation)" instance = instance dataset_dir = SCFLP_DIR seed = seed time_limit = time_limit build_only = build_only
...
if build_only
    @info "SCFLP knapsack script (CPLEX, Bodur formulation) build completed without solve." instance = instance
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "SCFLP knapsack script (CPLEX, Bodur formulation) finished" instance = instance termination_status = env.termination_status objective_value = obj_value
end
