...
if build_only
    @info "SCFLP knapsack script (CPLEX, original data) build completed without solve." instance = instance
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "SCFLP knapsack script (CPLEX, original data) finished" instance = instance termination_status = env.termination_status objective_value = obj_value
end
