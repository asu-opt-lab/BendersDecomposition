...
@info "UFLP knapsack script (Gurobi)" instance = instance seed = seed time_limit = time_limit threads = threads build_only = build_only
...
if build_only
    @info "UFLP knapsack script (Gurobi) build completed without solve." instance = instance
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "UFLP knapsack script (Gurobi) finished" instance = instance termination_status = env.termination_status objective_value = obj_value
end
