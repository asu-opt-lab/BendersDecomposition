...
EXPERIMENT_VERSION="1instance_new"
SEED="1"
HOUR="04"
TIME_LIMIT="14400"
ORACLE="knapsack"
EXPERIMENT_DESCRIPTION="${HOUR} hr, seed = ${SEED}, oracle = ${ORACLE}"
...
cp -r "polardcglp/scripts/classical/${SHELL_FILE_NAME}" "${OUTPUT_DIR}/${SHELL_FILE_NAME}"
...
for scenario_size in "${SCENARIO_SIZES[@]}"; do
    ...
    echo "julia --project=polardcglp polardcglp/scripts/classical/${FILE_NAME} --instance=${instance} --seed=${SEED} --time_limit=${TIME_LIMIT} --threads=${THREADS} --oracle=${ORACLE}" >> "${JOBSCRIPT_FILE}"
