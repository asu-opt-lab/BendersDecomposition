...
for instance_no in "${instance_nos[@]}"; do
    for snip_no in "${snip_nos[@]}"; do
        for budget in "${budgets[@]}"; do
            ...
            echo "julia --threads=${JULIA_THREADS} --project=polardcglp polardcglp/scripts/classical/${FILE_NAME} --instance_no=${instance_no} --snip_no=${snip_no} --budget=${budget} --seed=${SEED} --time_limit=${TIME_LIMIT} --output_dir=${OUTPUT_DIR}" >> "${JOBSCRIPT_FILE}"
