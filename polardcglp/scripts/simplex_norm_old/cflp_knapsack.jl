...
@info "SimplexNormOldDCGLP hard CFL script" instance = instance seed = seed time_limit = time_limit frequency = frequency reuse_dcglp = reuse_dcglp build_only = build_only
...
if build_only
    @info "SimplexNormOldDCGLP hard CFL script build completed without solve." instance = instance
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "SimplexNormOldDCGLP hard CFL script finished" instance = instance termination_status = env.termination_status objective_value = obj_value
end
