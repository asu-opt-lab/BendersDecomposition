...
@info "SimplexNormOldDCGLP hard UFL script" instance = instance seed = seed time_limit = time_limit frequency = frequency threads = threads reuse_dcglp = reuse_dcglp strengthened = strengthened lift = lift build_only = build_only
...
oracle_param = SimplexNormOldDCGLPParam(
    dcglp_param;
...
disjunctive_oracle = SimplexNormOldDCGLPOracle(master, typical_oracles, oracle_param)
...
if build_only
    @info "SimplexNormOldDCGLP hard UFL script build completed without solve." instance = instance
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "SimplexNormOldDCGLP hard UFL script finished" instance = instance termination_status = env.termination_status objective_value = obj_value
end
