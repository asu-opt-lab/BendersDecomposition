...
ROUND_DESCRIPTION="SimplexNormDCGLP SNIP, classical separable oracle, freq500, 56 cores SLURM, julia --threads=8, CPLEX"
EXPERIMENT_VERSION="new_true_parallel_1"
SEED="1"
HOUR="01"
TIME_LIMIT="14400"
...
STRENGTHENED="true"
LIFT="false"
EXPERIMENT_DESCRIPTION="${HOUR} hr, seed = ${SEED}, simplex_norm_dcglp"
...
cp -r polardcglp/scripts/simplex_norm_dcglp/${SHELL_FILE_NAME} "${OUTPUT_DIR}/${SHELL_FILE_NAME}"
...
for instance_no in "${instance_nos[@]}"; do
    for snip_no in "${snip_nos[@]}"; do
        for budget in "${budgets[@]}"; do
            ...
            echo "julia --threads=${JULIA_THREADS} --project=polardcglp polardcglp/scripts/simplex_norm_dcglp/${FILE_NAME} --instance_no=${instance_no} --snip_no=${snip_no} --budget=${budget} --seed=${SEED} --time_limit=${TIME_LIMIT} --dcglp_time_limit=${DCGLP_TIME_LIMIT} --dcglp_iter_limit=${DCGLP_ITER_LIMIT} --dcglp_halt_limit=${DCGLP_HALT_LIMIT} --frequency=${FREQUENCY} --reuse_dcglp=${REUSE_DCGLP} --strengthened=${STRENGTHENED} --lift=${LIFT}" >> "${JOBSCRIPT_FILE}"
