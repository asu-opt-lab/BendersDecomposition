...
if build_only
    @info "SimplexNormDCGLP SNIP script build completed without solve." instance_no = instance_no snip_no = snip_no budget = budget
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "SimplexNormDCGLP SNIP script finished" instance_no = instance_no snip_no = snip_no budget = budget termination_status = env.termination_status objective_value = obj_value
end
