...
time_limit = get_float_option(options, "time_limit", 3600.0)
root_time_limit = get_float_option(options, "root_time_limit", 3600.0)
dcglp_time_limit = get_float_option(options, "dcglp_time_limit", 3600.0)
...
@info "DirectionalPolarDCGLP CFL script (classical oracle)" instance = instance seed = seed time_limit = time_limit root_time_limit = root_time_limit frequency = frequency reuse_dcglp = reuse_dcglp strengthened = strengthened build_only = build_only split_rule = split_rule_name core_x_mode = core_x_mode
...
if build_only
    @info "DirectionalPolarDCGLP CFL classical script build completed without solve." instance = instance
else
    solve!(env)
    obj_value = try
        env.obj_value
    ...
    @info "DirectionalPolarDCGLP CFL classical script finished" instance = instance termination_status = env.termination_status objective_value = obj_value
end
