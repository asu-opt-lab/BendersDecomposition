...
function pplot()
    ...
    for k in [700]
        ...
        for i in 1:10
            iter_ = 0
            all_time = 0
            filename = "results3/Split_all_L1_iter2_1hr/result_f$(k)-c$(k)-r5.0-p$(i).csv"
            data = CSV.read("$(filename)", DataFrame)
            std_LB = data.LB[2]
            std_time = data.sub_time[1]
            filename = "results3/Advanced_4hr/result_f$(k)-c$(k)-r5.0-p$(i).csv"
            data = CSV.read("$(filename)", DataFrame)
            LBs = data.LB
...
df = pplot()
CSV.write("results3/data_700.csv", df)