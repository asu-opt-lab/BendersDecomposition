...
# OUTPUT_DIR="experiments/snip_milp_test"
OUTPUT_DIR="experiments_final_14/snip_milp"
...
for instance in "${instances[@]}"; do
    for snip in "${snipno[@]}"; do
        for budget in "${budgets[@]}"; do
            ...
            echo "julia --project=. scripts/milp_snip.jl --instance ${instance} --snip_no ${snip} --budget ${budget} --output_dir ${OUTPUT_DIR} --config ${config_path}" >> "${JOBSCRIPT_FILE}"
