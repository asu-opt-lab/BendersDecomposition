...
ROUND_DESCRIPTION="1123_knapsack_PDHG_relaxed_tol(1e-2)"
EXPERIMENT_DESCRIPTION="Crossover(1), Threads(7), LPWarm(0), PDHGGPU(1), PDHGAbs/RelTol(1e-2)"

# How many jobs to run at once (upper bound)
CONCURRENCY="${CONCURRENCY:-1}"

# Pin to a specific GPU (default to GPU 1)
PIN_TO_GPU="${PIN_TO_GPU:-1}"
...
# ── dispatch jobs (pin to GPU 1) ──────────────────────────────────────────────
run_on_any_gpu() {
  local inst="$1"
  local job="$2"
  local gpu="${PIN_TO_GPU}"
