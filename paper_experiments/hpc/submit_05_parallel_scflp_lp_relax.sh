...
ROUND_VERSION="05_parallel_scflp_lp_relax"
ROUND_DESCRIPTION="Parallel SCFLP LP relaxation, generated 50x50x512 with original stochastic generator, separable oracle"
...
ENV_NAME="seq"
SOLVER="gurobi"
EXPERIMENT_DESCRIPTION="${HOUR} hr, repeats = ${REPEATS}, env = ${ENV_NAME}, oracle = ${ORACLE}, LP relaxation"

FILE_NAME="05_parallel_scflp_lp_relax.jl"
SHELL_FILE_NAME="submit_05_parallel_scflp_lp_relax.sh"
SOLVER_THREADS=1
MEM="100G"
PARTITION="general"
QOS="grp_gbyeon"
...
for repeat in $(seq 1 "${REPEATS}"); do
    for instance in "${scflp_instances[@]}"; do
        for JULIA_THREADS in "${julia_threads_list[@]}"; do
            JOB_NAME="scflp_${instance}_${ENV_NAME}_${ORACLE}_lp_relax_jt${JULIA_THREADS}_r${repeat}"
