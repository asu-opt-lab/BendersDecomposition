...
ROUND_DESCRIPTION="Oracle ablation, BendersSeq, UFLP 250x250, CFLP 200x200, SCFLP 100x200"
...
for repeat in $(seq 1 "${REPEATS}"); do
    ...
    for instance in "${scflp_instances[@]}"; do
        for oracle in "classical" "unified" "pareto" "cfl_knapsack"; do
            JOB_NAME="scflp_${instance}_${oracle}_r${repeat}"
            JOBSCRIPT_FILE="${OUTPUT_DIR}/${JOB_NAME}.sh"
            JOB_OUTPUT_DIR="${OUTPUT_DIR}/${JOB_NAME}"
            mkdir -p "${JOB_OUTPUT_DIR}"
            ...
            echo "module load gurobi" >> "${JOBSCRIPT_FILE}"
            echo "cd ${REPO_ROOT}" >> "${JOBSCRIPT_FILE}"
            echo "julia --project=paper_experiments paper_experiments/scripts/${FILE_NAME} --output_dir=${JOB_OUTPUT_DIR} --time_limit=${TIME_LIMIT} --gap_tolerance=${GAP_TOLERANCE} --solver_threads=${THREADS} --repeats=${REPEATS} --repeat_index=${repeat} --problem=scflp --instance=${instance} --oracle=${oracle}" >> "${JOBSCRIPT_FILE}"
            sbatch "${JOBSCRIPT_FILE}"
            rm "${JOBSCRIPT_FILE}"
        done
    done
