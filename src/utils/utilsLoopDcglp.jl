...
Parameter container for the Dual Cut Generating Linear Program (DCGLP).

`DcglpParam` controls the optimization settings and stopping criteria used when
solving the DCGLP within disjunctive Benders oracles (e.g., [`SplitOracle`](@ref)).
It specifies the solver configuration, numerical tolerances, and iteration
limits for the DCGLP cutting-plane loop.
...
# Fields
- `optimizer::MOI.OptimizerWithAttributes`: Optimizer (with attributes) used to solve the DCGLP. This should be a fully configured MathOptInterface optimizer (e.g., CPLEX, Gurobi, HiGHS) with any desired solver-specific attributes already set.
- `time_limit::Float64`: Maximum wall-clock time (in seconds) allowed for solving the DCGLP.
- `gap_tolerance::Float64`: Optimality gap tolerance for terminating the DCGLP.
- `halt_limit::Int`: Maximum number of consecutive non-improving DCGLP iterations before termination.
- `iter_limit::Int`: Maximum number of DCGLP iterations.
- `verbose::Bool`: Whether to print DCGLP iteration information.

# Notes
- The DCGLP is typically solved repeatedly during cut generation; tuning time_limit, halt_limit, and iter_limit can have a significant impact on overall performance.
- The optimizer must support the constraints and variable types required by the DCGLP formulation.
