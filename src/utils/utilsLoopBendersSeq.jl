...
Parameter container for the standard sequential Benders decomposition
environment.

`BendersSeqParam` controls global stopping criteria and logging behavior for
sequential Benders environments (such as [`BendersSeq`](@ref)). 
...
# Fields
- `time_limit::Float64`: Maximum wall-clock time (in seconds) allowed for the algorithm.
...
# Notes
- Termination is triggered when the optimality gap falls below `gap_tolerance`, the time limit is exceeded, or (halt) iteration limits are reached.
- This parameter type is intended for the basic sequential Benders algorithm.
- Use `BendersSeqInOutParam` for stabilized In–Out variants.
See also: `BendersSeq`, `AbstractBendersSeqParam`
"""
