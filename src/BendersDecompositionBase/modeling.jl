...
mutable struct GenericSubProblem <: AbstractSubProblem 
    model::Model
    fixed_x_values::Union{Vector{Float64}, Float64}
...
macro sub_problem(env_sym, model_expr, body)
    ...
    return quote
        let
            ...
            for con_ref in all_constraints($(esc(model_expr)), include_variable_in_set_constraints=false)
                push!(other_constraints, con_ref)
            end
            # TODO: change to integer_variables
            fixed_x_values = zeros(length(var_dict[:x]))
            # Add fixed_x_constraints
            fixed_x_cons = @constraint($(esc(model_expr)), var_dict[:x] .== fixed_x_values)
