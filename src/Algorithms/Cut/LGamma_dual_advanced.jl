function solve_DCGLP(
    ...
    while true
        ...
        # push!(v̂ₓs, v̂ₓ)
        # push!(v̂ₜs, v̂ₜ)
        N,M = size(bsp_env1.model[:π3])
        ##################### BSP1 #####################
        if k̂₀ != 0 #|| k == 1
            # @info "k̂ₜ/k̂₀ = $(k̂ₜ/k̂₀)"
            @objective(bsp_env1.model, Max, sum(bsp_env1.model[:π1])*k̂₀ - sum(bsp_env1.model[:π2])*k̂₀ - sum(k̂ₓ[i]*bsp_env1.model[:π3][i,j] for i in 1:N, j in 1:M) - bsp_env1.model[:π0]*k̂ₜ)
            ...
            _π1 = value.(bsp_env1.model[:π1])
            _π2 = value.(bsp_env1.model[:π2])
            _π3 = value.(bsp_env1.model[:π3])
            _π0 = value(bsp_env1.model[:π0])
            push!(conπpoints1, [_π1,_π2,_π3,_π0])
        ...
        if v̂₀ != 0 #|| k == 1
            ...
            @objective(bsp_env2.model, Max, sum(bsp_env2.model[:π1])*v̂₀ - sum(bsp_env2.model[:π2])*v̂₀ - sum(v̂ₓ[i]*bsp_env2.model[:π3][i,j] for i in 1:N, j in 1:M) - bsp_env2.model[:π0]*v̂ₜ)
            ...
            # if status2 == FEASIBLE_POINT
                ...
                _π1 = value.(bsp_env2.model[:π1])
                _π2 = value.(bsp_env2.model[:π2])
                _π3 = value.(bsp_env2.model[:π3])
                _π0 = value(bsp_env2.model[:π0])
                push!(conπpoints2, [_π1,_π2,_π3,_π0])
