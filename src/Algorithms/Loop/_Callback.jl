function run_Benders_callback(
    ...
    set_attribute(master_env.model, MOI.UserCutCallback(), my_callback_function)
...
function user_callback(cb_data)
    ...
    # println(depth[])
    if status == MOI.CALLBACK_NODE_STATUS_FRACTIONAL && number_of_splitproblem_solves <= 10
        # @info "user"
        ...
        γ₀, γₓ, γₜ = generate_cut_callback(Master_env, Sub_env, SPLIT_CUTSTRATEGY)
