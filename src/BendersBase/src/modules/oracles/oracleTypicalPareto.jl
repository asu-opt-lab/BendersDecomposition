...
where:
- `ξ^*` is the optimal objective value from the classic subproblem.
- `x^*` is the current master solution.
- `ϵ` is `pareto_tol` defined in [`ParetoOracleParam`](@ref).
- `x_0` is the core point (dynamically updated if λ < 1.0).
- `σ` is the primal variable associated with the Pareto-optimality constraint.
...
# Arguments
The arguments of ParetoOracle are same with [`ClassicalOracle`](@ref), whereas `pareto_model` is additionally constructed inside the constructor.
