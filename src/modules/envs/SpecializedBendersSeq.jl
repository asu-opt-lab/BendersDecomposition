...
`SpecializedBendersSeq` implements the specialized sequential Benders
decomposition algorithm introduced by Fang et al. (2025). The method addresses
MILP master problems by repeatedly solving LP relaxations of the master under a
restricted, algorithmically valid configuration of a [`SplitOracle`](@ref).
"""
