...
function build_strategy_dcglp(strategy::DirectionalPolarStrategy, master::AbstractMaster, param::DcglpOracleParam{DirectionalPolarStrategy})
    ...
    # The `tau_var` coefficient on `conx`/`cont` is updated each call via
    ...
    @constraint(
        ...
        dcglp[:omega_x][1, j] + dcglp[:omega_x][2, j] + 0.0 * tau_var == strategy.core_point_x[j],
    )
    @constraint(
        dcglp,
        cont[j in 1:master.dim_t],
        dcglp[:omega_t][1, j] + dcglp[:omega_t][2, j] + 0.0 * tau_var == strategy.core_point_t[j],
    )
...
function prepare_dcglp_call!(strategy::DirectionalPolarStrategy, oracle::DirectionalPolarOracle, x_value::Vector{Float64}, t_value::Vector{Float64})
    update_direction_constraints!(oracle, x_value, t_value, strategy.last_direction_x, strategy.last_direction_t)
    return nothing
end

dcglp_tau_value(::DirectionalPolarStrategy, dcglp::Model) = value(dcglp[:tau_var])
...
function build_dcglp_disjunctive_cut(
    ...
    isempty(strategy.last_direction_x) &&
        throw(AlgorithmException("DirectionalPolarStrategy: no direction vector cached. Call prepare_dcglp_call! first."))

    gamma_x = dual.(dcglp[:conx])
    gamma_t = dual.(dcglp[:cont])
    normalize_directional_duals!(gamma_x, gamma_t, strategy.last_direction_x, strategy.last_direction_t; zero_tol = common.zero_tol)
