...
mutable struct CFLPDCGLPEnv <: AbstractDCGLPEnv
    model::Model
    ifsolved::Bool
    masterconπpoints1
    masterconπpoints2
...
function DCGLP(sub_env::CFLPSplitSubEnv, a::Vector{Int}, b::Int, norm_type::StandardNorm; solver::Symbol=:Gurobi) 
...
function DCGLP(sub_env::CFLPSplitSubEnv, a::Vector{Int}, b::Int, norm_type::GammaNorm; solver::Symbol=:CPLEX)
    ...
    @variable(model, τ)
    @variable(model, k₀>=0)
    @variable(model, kₓ[1:N]>=0)
    @variable(model, kₜ)
    @variable(model, v₀>=0)
    @variable(model, vₓ[1:N]>=0)
    @variable(model, vₜ)
