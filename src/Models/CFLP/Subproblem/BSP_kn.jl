...
mutable struct CFLPBSPKNEnv <: AbstractSubEnv
    model::Model
    knapsack_subproblems
end

#### Ordinary version
function generate_BSPProblem_KN(data::CFLPData; solver::Symbol=:Gurobi)
    ...
    knapsack_problems = generate_knapsack_subproblems(data)
    return CFLPBSPKNEnv(model,knapsack_problems)
end