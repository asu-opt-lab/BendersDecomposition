...
"""
function solve_dcglp!(oracle::DisjunctiveOracle, x_value::Vector{Float64}, t_value::Vector{Float64}; rtol = 1e-8, atol = 1e-9, time_limit = time_limit, throw_typical_cuts_for_errors = true, include_disjuctive_cuts_to_hyperplanes = true)
    log = DcglpLog()
    ...
    while true
        ...
        state.total_time = @elapsed begin
            ...
            for i in 1:2
                ...
                state.oracle_times[i] = @elapsed begin
                    if ω_0[i] >= atol
                        state.is_in_L[i], hyperplanes_a, state.f_x[i] = generate_cuts(typical_oracles[i], clamp.(ω_x[i] / ω_0[i], 0.0, 1.0), ω_t[i] / ω_0[i], tol = atol / ω_0[i], time_limit = get_sec_remaining(log.start_time, time_limit))
        ...
        check_lb_improvement!(state, log; zero_tol = atol)
