export CFLKnapsackOracleParam, CFLKnapsackOracle
...
Parameter type for [`CFLKnapsackOracle`](@ref).

`CFLKnapsackOracleParam` is an alias of [`BasicOracleParam`](@ref) and controls
...
Oracle for the Capacitated Facility Location (CFL) subproblem based on a
knapsack-based separation procedure.

This oracle implements the separation approach proposed by  
Fischetti, M., Ljubić, I., & Sinnl, M. (2016), *Benders decomposition without
separability: A computational study for capacitated facility location
problems*, European Journal of Operational Research, 253(3), 557–569.
...
The arguments and stored fields of CFLKnapsackOracle follow the same interface
and conventions as [`ClassicalOracle`](@ref).
"""
